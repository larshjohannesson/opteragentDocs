...
- **Total ServerApi-active customer count is flat (~575-597) across the entire 90 days** — there is no visible dip in *customer presence* through late June/July. An earlier informal read of a (since-discarded, methodologically flawed) chart suggested a summer-vacation dip in this data; that claim is retracted. If the Swedish-summer-vacation effect on usage is real, it would show up in *request/activity volume per customer*, not in whether a customer had any activity at all that week — that volume-based view has not been built yet (see Recommended next steps).
...
- **Not yet built**: a usage-volume view (request/log-row counts per customer over time, not just customer presence) to properly test the Swedish-summer-vacation effect on usage, which the corrected customer-presence data does not show (see "Evidence" section above).
...
7. **Build a usage-volume view** (total request/log-row count per customer per week, not just customer presence) to properly confirm or rule out the Swedish-summer-vacation effect on usage — the customer-presence data gathered so far is flat through the summer and cannot speak to this.
